...
function crearCard(emp) {
    ...
    <div class="emprendimiento-card__content">
        ...
        <div class="emprendimiento-card__footer">
            <div class="emprendimiento-card__contact">
                ${telefono ? `
                    <a href="https://wa.me/${telefono}" target="_blank" class="emprendimiento-card__contact-btn emprendimiento-card__contact-btn--whatsapp" title="WhatsApp">
                ...
                ${instagram ? `
                    <a href="https://instagram.com/${instagram}" target="_blank" class="emprendimiento-card__contact-btn emprendimiento-card__contact-btn--instagram" title="Instagram">
                ...
                ${facebook ? `
                    <a href="${formatearFacebookURL(facebook)}" target="_blank" class="emprendimiento-card__contact-btn emprendimiento-card__contact-btn--facebook" title="Facebook">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                    </a>` : ''}
                ${email ? `
                    <a href="mailto:${escapeHtml(email)}" class="emprendimiento-card__contact-btn emprendimiento-card__contact-btn--email" title="Email">
    ...
    card.querySelectorAll('.emprendimiento-card__contact-btn').forEach(btn => {
        ...
        if (coords) {
            ...
            const popupContent = `
                <div class="map-popup">
                    ...
                    <button class="map-popup__btn" data-index="${index}">${escapeHtml(i18n.t('modal.viewDetails'))}</button>
                </div>
            `;
            
            marker.bindPopup(popupContent, { maxWidth: 250, className: 'custom-popup' });
            marker.on('popupopen', function() {
                const btn = marker.getPopup().getElement().querySelector('.map-popup__btn');
                if (btn) btn.addEventListener('click', () => window.abrirModalDesdeIndex(parseInt(btn.dataset.index, 10)));
            });
